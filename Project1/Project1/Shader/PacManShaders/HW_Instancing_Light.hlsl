...
{
    PixelInput output;
    output.pos = mul(input.pos, input.trasnform);
    ...
    output.normal = mul(input.normal, (float3x3) input.trasnform);
    output.tangent = mul(input.tangent, (float3x3) input.trasnform);
